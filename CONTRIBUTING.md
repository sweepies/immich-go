...
Our repository focuses on fast feedback CI for linting, unit tests, and build validation.

### Fast Feedback Workflow (`.github/workflows/pr-fast-feedback.yml`)

**Runs automatically on pull requests and main/develop pushes** - No secrets required.

Provides quick validation in a few minutes:
- **Linting:** `golangci-lint` for code quality checks
- **Unit Tests:** Race detection on Linux, standard tests on Windows
- **Build Check:** Cross-platform compilation validation

Documentation-only changes are typically skipped for efficiency.
