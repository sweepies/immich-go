	...
	for _, entry := range entries {
		...
		if s.icloudMetaPass && ext == icloudMetadataExt {
			if strings.HasSuffix(strings.ToLower(dir), "albums") {
				...
				if err != nil {
					s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, name), 0, journal.ErrorFileAccess, "error", err.Error())
				} else {
					s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, name), 0, journal.DiscoveredMetadata, "album", album)
				}
				continue
			}
			...
			if strings.HasPrefix(strings.ToLower(base), "photo details") {
				...
				if err != nil {
					...
					s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, name), 0, journal.DiscoveredMetadata)
				}
				continue
			}
		}
		...
		if s.config.BannedFiles.Match(name) {
			s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, entry.Name()), 0, journal.DiscoveredBanned, "reason", "banned file")
			continue
		}
		...
		if s.config.PicasaAlbum && isPicasaIni(base) {
			...
			if err != nil {
				...
				s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, name), 0, journal.DiscoveredMetadata, "album", album.Name)
			}
			continue
		}

		if s.deps.SupportedMedia.IsUseLess(name) {
			s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, entry.Name()), 0, journal.DiscoveredUnknown, "reason", "useless file")
			continue
		}
		...
		if mediaType == filetypes.TypeUnknown {
			s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, name), 0, journal.DiscoveredUnsupported, "reason", "unsupported file type")
			continue
		}

		switch mediaType {
		case filetypes.TypeUseless:
			s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, name), 0, journal.DiscoveredUnknown)
			continue
		...
		case filetypes.TypeSidecar:
			if s.config.IgnoreSideCarFiles {
				s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, name), 0, journal.DiscoveredSidecar, "reason", "sidecar file ignored")
				continue
			}
			s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, name), 0, journal.DiscoveredSidecar)
			continue
		}

		if !s.config.InclusionFlags.IncludedExtensions.Include(ext) {
			if info, err := fs.Stat(fsys, name); err == nil {
				s.deps.Processor.RecordAssetDiscardedImmediately(ctx, fshelper.NewFilename(fsys, name), info.Size(), journal.DiscardedFiltered, "extension not included")
			}
			continue
		}
		...
		if s.config.InclusionFlags.ExcludedExtensions.Exclude(ext) {
			if info, err := fs.Stat(fsys, name); err == nil {
				s.deps.Processor.RecordAssetDiscardedImmediately(ctx, fshelper.NewFilename(fsys, name), info.Size(), journal.DiscardedFiltered, "extension excluded")
			}
			continue
		}
		...
		default:
			...
			if err != nil {
				s.deps.Processor.RecordAssetError(ctx, fshelper.NewFilename(fsys, name), 0, journal.ErrorFileAccess, err)
				continue
			}
			if a != nil {
	...
	for _, entry := range entries {
		...
		if entry.IsDir() {
			if s.config.BannedFiles.MatchDir(name) {
				s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, name), 0, journal.DiscoveredBanned, "reason", "banned folder")
				continue
			}
...
func (s *FolderSource) processAssetMetadata(ctx context.Context, fsys fs.FS, a *assets.Asset, fsName, dir string, dirFiles map[string]string) {
	...
	if jsonName != "" {
		...
		if err != nil {
			s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, jsonPath), 0, journal.ErrorFileAccess, "error", err.Error())
		} else {
			...
			if bytes.Contains(buf, []byte("immich-go version")) {
				...
				if err != nil {
					s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, jsonName), 0, journal.ErrorFileAccess, "error", err.Error())
				} else {
					s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, jsonName), 0, journal.DiscoveredSidecar)
					md.File = fshelper.NewFilename(fsys, jsonName)
					a.FromApplication = a.UseMetadata(md)
					a.OriginalFileName = md.FileName
				}
			} else {
				s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, jsonName), 0, journal.DiscoveredSidecar)
				s.deps.Logger.Warn("JSON file detected but not from immich-go", "file", fshelper.NewFilename(fsys, jsonName))
			}
		}
	}
	...
	if xmpName != "" {
		...
		if err != nil {
			s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, xmpPath), 0, journal.ErrorFileAccess, "error", err.Error())
		} else {
			...
			if err != nil {
				s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, xmpName), 0, journal.ErrorFileAccess, "error", err.Error())
			} else {
				s.deps.Processor.RecordNonAsset(ctx, fshelper.NewFilename(fsys, xmpName), 0, journal.DiscoveredSidecar)
				md.File = fshelper.NewFilename(fsys, xmpName)
